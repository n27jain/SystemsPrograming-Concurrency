		...
		if (result == 0) {
			if (memcmp(chk->type, "IHDR", CHUNK_TYPE_SIZE) == 0) {
				...
				if (crc_computed == chk->crc) {
					...
					while (true) {
						free(chk->p_data);
						if (get_chunk_from_file(chk, fp, 0, SEEK_CUR) != 0) {
						...
						if (memcmp(chk->type, "IEND", CHUNK_TYPE_SIZE) == 0) {
							...
							if (calculate_chunk_crc(chk, &crc_computed) == 0) {
								if (crc_computed != chk->crc) {
									fprintf(stderr, "IEND chunk CRC error: computed %08lx, expected %08x\n", crc_computed, chk->crc);
									ret = -1;
								}
							} else {
								fprintf(stderr, "error: failed to calculate IEND CRC\n");
								ret = -1;
							}
							break;
					...
					if (ret == 0 && !found_iend) {
						fprintf(stderr, "error: IEND chunk missing\n");
						ret = -1;
					}
				} else {
